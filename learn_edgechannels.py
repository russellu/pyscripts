...
for i in np.arange(0,63):
    ...
    labs.append(montage.ch_names[np.int(newinds[i])])


heartinds = [12,13,28,29,31]
x = x[0:64,:]

# try a single channel first
epoch_sz = 350
epochx = np.zeros([5,epoch_sz,x.shape[1] - epoch_sz])
epochy = np.zeros([64,x.shape[1] - epoch_sz])

for i in np.arange(0,x.shape[1] - epoch_sz-1):
    epochx[:,:,i] = x[heartinds,i:i+epoch_sz]
    epochy[:,i] = x[:,i+epoch_sz+1]


epochx = np.reshape(epochx,[5*epoch_sz,epochx.shape[2]])

RESHAPED = epochx.shape[0]
OPTIMIZER = Adam(lr=0.00005)

model = Sequential()
model.add(Dense(100, input_shape=(RESHAPED,)))#kernel_regularizer=regularizers.l1(0.00001)
...
historY = model.fit(epochx.transpose(), epochy.transpose(), 
                    batch_size=300, epochs=100, validation_split=0.2)
