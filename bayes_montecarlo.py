...
with model:
    ...
    grass_wet_p = pm.Deterministic('grass_wet_p', pm.math.switch(rain, \
            pm.math.switch(sprinkler, 0.99, 0.80), pm.math.switch(sprinkler, 0.90, 0.0)))
    
    grass_wet = pm.Bernoulli('grass_wet', grass_wet_p, observed=np.array([1]), shape=1)

    trace = pm.sample(5000, step=[pm.BinaryGibbsMetropolis([rain, sprinkler])], \
                                  tune=tune, random_seed=124,cores=1)
